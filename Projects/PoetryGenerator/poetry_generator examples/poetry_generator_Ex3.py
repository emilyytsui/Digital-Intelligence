...
def generate_line(rhyme, sz, ln):
    #print("RHYME (" + str(sz) + "." + str(ln) + "):", rhyme)
    line_done = False
        ...
        for w in line_list:
            ...
            for s1 in stresses:
                for s2 in sl:
                    temp.append(s1 + s2)
            stresses = temp
        print("STRESSES (" + str(sz) + "." + str(ln) + "):", stresses)
        print("LINE_LIST (" + str(sz) + "." + str(ln) + "):", line_list)
...
def generate_poem():
    # Example 3: Shakespearean Sonnet
    lines_list = []
    ...
    for j in range(1, 4):
        stanza_list = []
        ln = 1
        stanza_list.append(generate_line(None, j, ln))
        ln += 1
        stanza_list.append(generate_line(None, j, ln))
        for i in range(2):
            ln += 1
            stanza_list.append(generate_line(stanza_list[i].split()[-1], j, ln))
        lines_list.extend(stanza_list)
        
    # two additional lines, rhyme pattern GGd
    lines_list.append(generate_line(None, 4, 1))
    lines_list.append(generate_line(lines_list[-1].split()[-1], 4, 2))
