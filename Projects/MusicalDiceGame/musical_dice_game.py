...
def minuet_filename(mmid):
    #return "/Users/emilytsui/Desktop/sbu/IAE 101/mozart/M" + mmid + ".wav"
...
def trio_filename(tmid):
    #return "/Users/emilytsui/Desktop/sbu/IAE 101/mozart/T" + tmid + ".wav"
...
def construct_waltz():
    ...
    minuetWaveObject = []
    for i in minuet:
        minuetWaveObject.append(simpleaudio.WaveObject.from_wave_file(minuet_filename(i)))
    
    buffer = b""
    for i in minuetWaveObject:
        buffer += i.audio_data
        nc = i.num_channels
        bps = i.bytes_per_sample
        sr = i.sample_rate

        #playerObject = i.play()
        playerObject = simpleaudio.play_buffer(buffer, nc, bps, sr)
        playerObject.wait_done()

    # Same loop for selections from the trio_table
    #
    trioWaveObject = []
    for i in trio:
        trioWaveObject.append(simpleaudio.WaveObject.from_wave_file(trio_filename(i)))

    buffer = b""
    for i in trioWaveObject:
        buffer += i.audio_data
        nc = i.num_channels
        bps = i.bytes_per_sample
        sr = i.sample_rate
